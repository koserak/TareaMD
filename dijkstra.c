...
double dijkstra(Ciudad *ciudad, int inicio_id, int fin_id,
    ...
    for (int iter = 0; iter < n; iter++) {
        /* Nodo no visitado con distancia minima */
        int u = -1;
        for (int j = 0; j < n; j++) {
        ...
        visitado[u] = true;
        if (u == fin_id) break;  /* ya encontramos el destino */
    ...
    if (distancia[fin_id] >= INF) {
        *largo_camino = 0;
        return -1.0;   /* no hay ruta */
    }
 
    /* Reconstruir camino (de fin a inicio, luego invertir) */
